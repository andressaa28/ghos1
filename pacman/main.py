    ...
    while rodando:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.JOYBUTTONDOWN and joystick is not None:
                if event.button == 7:  # Start / Options = ENTER
                    rodando = False
                elif event.button == 1:  # B / O = ESC
                    pg.quit()
                    quit()
            ...
            if event.type == pg.JOYAXISMOTION and joystick is not None:
                if event.axis == 0:  # eixo horizontal
                    if event.value < -0.5:
                        ...
                        selecao = (selecao + 1) % len(personagens)
                elif event.axis == 1:  # eixo vertical
                    if event.value < -0.5:
                        selecao = (selecao - cols) % len(personagens)
                    elif event.value > 0.5:
                        selecao = (selecao + cols) % len(personagens)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_RETURN:  # Vai para os créditos
                    ...
                    tela_inicial()
                    selecao = escolher_personagem(joystick)  # se quiser reaparecer seleção
                    rodando = False
            ...
            if event.type == pg.JOYBUTTONDOWN and joystick is not None:
                if event.button == 7:  # Start / Options = ENTER
                    rodando = False
                elif event.button == 6:  # Back / Share = R (menu)
                    tela_inicial()
                    selecao = escolher_personagem(joystick)
                    rodando = False
                elif event.button == 1:  # B / O = ESC
                    pg.quit()
                    quit()
    ...
    while rodando:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
             ...
             # --- Joystick ---
            if event.type == pg.JOYBUTTONDOWN and joystick is not None:
                if event.button == 7:  # Start = ENTER
                    rodando = False
                elif event.button == 6:  # Back = R
                    rodando = False
                elif event.button == 1:  # B / O = ESC
                    pg.quit(); quit()  
...
while True:  # Loop principal (menu -> jogo -> menu)
    ...
    while True:  # Loop da partida
        for event in pg.event.get():
            ...
            # --- Joystick (botões) ---
            if event.type == pg.JOYBUTTONDOWN and joystick is not None:
                if event.button == 0: jogo.move("up")      # A / X
                elif event.button == 1: jogo.move("down")  # B / O
                elif event.button == 2: jogo.move("left")  # X / Quadrado
                elif event.button == 3: jogo.move("right") # Y / Triângulo
                elif event.button == 7:  # Start = ENTER
                    pass  # (pode usar como "confirmar" em alguma lógica)
                elif event.button == 6:  # Back = R (menu)
                    sair_para_menu = True
                elif event.button == 1:  # B / O = ESC (sair do jogo)
                    pg.quit()
                    quit()
